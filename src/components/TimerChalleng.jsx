...
export default function TimerChalleng({title, targetTime }){
    ...
    return(
        <>
        <ResultModal ref={dialog} targetTime={targetTime} remainingTime = {timeRemining} onReset={handleReset} result="lost" />
        <section className="challenge">
            ...
            <p>
                <button onClick={timeIsActive ? handleStop : handleStart}>
                    {timeIsActive ? 'stop' : 'start'} challeng
                </button>
            </p>
            <p className={timeIsActive ? 'active' : undefined}>
                {timeIsActive? 'Timer is runnin...' : 'Timer inactive'} 
            </p>
        </section>
        </>
    )
